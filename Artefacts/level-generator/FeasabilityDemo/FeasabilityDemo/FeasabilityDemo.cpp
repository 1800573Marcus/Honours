...
int fitness(int map[20][20]) {
	...
	int distance = 1; //counts how long the path is
	int distancelooped = 0;//counts how many loops the distance value hasnt changed for in case it gets stuck
	bool foundtarget = false; 
	bool nextsteppossible = true;

	int ends = 0;// counts end points
	int rooms = 0;//counts the rooms in the level

	for (int i = 0; i < 20; i++) {// counts haw many end points the level has
		for (int j = 0; j < 20; j++) {
			if (map[i][j] == -3)
				ends++;
		}
	}
	for (int i = 0; i < 20; i++) { // counts haw many rooms the level has
		for (int j = 0; j < 20; j++) {
			if (map[i][j] == -5)
				rooms++;
		}
	}
	if (ends > 1) {// if the map has too many rooms or end points it returns a failure
		return 0;
	}
	if (rooms < 2) {
	...
	while (foundtarget == false) { //until target found
			...
			for (int j = 0; j < 20; j++) {//check every coordinate
				...
				if (map[i][j] == distance) { //find where the path has reached
					for (int h = 0; h < 4; h++) {  //check coords around newest point
						...
						else if (map[i + directions[h][0]][j + directions[h][1]] == -4) {//if the coord is in a room mark it for the next loop
							map[i + directions[h][0]][j + directions[h][1]] = distance + 1;

						}
						else if (map[i + directions[h][0]][j + directions[h][1]] == -5) {//if the coord is in a room mark it for the next loop
							map[i + directions[h][0]][j + directions[h][1]] = distance + 1;
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 20; j++) {
				if (map[i][j] == distance) {//if there is no where left to go the pathfinder gives up and returns a failure
...
void geneicAlgorithm(int map[20][20][20]) {
	//This function goes unused as the project changed. It is here to show the work even if it isnt used.
...
int branchrooms(int map[10][20][20]) {
	//This function is unfinished as it had to be cut due to time constraints. it was intended to add branches off of the main path generated for the final maps.
...
int main()
{
   ...
   while (goodmaps < 6){
	...
	//for (int h = 0; h < 20; h++) {
		...
		for (int i = 1; i < 3; i++) {
			for (int j = 1; j < 3; j++) {// mark starting room
		...
		currentx += randomDirection[0] * 4; // move position out of room
		currenty += randomDirection[1] * 4;
		newmap[currentx - randomDirection[0]][currenty - randomDirection[1]] = -1;
		newmap[currentx ][currenty] = -1; // mark position


		while (maxtunnels > 0) { // wander until max amount of tunnels has been generated 
			
			spacecheck = true; // assume there is room to place a room
			for (int i = 1; i < 4; i++) {
				for (int j = 1; j < 4; j++) {
					if (newmap[currentx + i][currenty + j] == -4) { // check if there is space for a room,
						spacecheck = false;
					}
			...
			if (spacecheck == true) {// if there is space, place a new room
			...
			if (newmap[currentx][currenty] == -5) {//direction control for room placment
				...
				while ((randomDirection[0] == -lastDirection[0] && randomDirection[1] == -lastDirection[1]) || (randomDirection[0] == lastDirection[0] && randomDirection[1] == lastDirection[1])) { //loop until i new direction that isnt backwards is chosen
					...
					if (stuckdirections < 10) {// if stuck for too long, give up
						stuckdirections = 0;
						stuckcount = 20;
						break;
					}
			...
			if (newmap[currentx][currenty] == -5) {// move point putside of room if they are in the centre of one
			...
			for (int i = 1; i <= randlength + 2; i++) {// check if the tunnel fits
			...
			if (badtunnel == true) {
				...
				stuckcount++; // if the tunnel is bad it means we are stuck, if stuck for too long the algorithm stops
			}
			if (stuckcount >= 20) {
				maxtunnels = 0;
			}
		...
		for (int i = 0; i < 20; i++) {
			...
			for (int j = 0; j < 20; j++) {
				if (map[h][i][j] == -1) { //output an image marking the start, end and tunnels with appropriate colours
					...
					img << 20 << " " << 20 << " " << 0 << endl;//room centre
					roomtype = rand() % 6;
					cout << endl << "room type : " << roomtype << endl; //randomly assign room type.
